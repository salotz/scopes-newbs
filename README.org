...
characters. You can even get the strings from them (see Type Casting
below):
...
Wait.. but I thought you said that we couldn't have a ~'~ in a symbol?

Above we were assigning values to symbols and because the ~'~ (and the
~`~) character have priveleged roles in the language there are
restrictions on them being in symbols themselves.

A full explanation is quite involved and is really a topic for Level 3
and any Lisper or Schemer will already know about this (except for the
~`~) so we will delay a more complete explanation until then.
