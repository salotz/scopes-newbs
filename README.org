...
The storage location (i.e. stack or data-segment (AKA heap)) of the
value generated from an expression is determined by the kind of
expression.
...
# NOTE: not sure this comment about the data-segment is actually
# correct. It might be another construct that is a part of the final
# executable binary.

# The "data-segment" is all the memory that is not in the stack and can
...
# like scoping rules to know when data is no longer needed. It is
# typically called the "heap" due to the use of a heap datastructure in
# managing (i.e. *allocating*) this data in C, although any allocating
# algorithm in principle can be used.

# The "data-segment" part of programming in languages with manual memory
# management is a large part of what makes them so challenging to use
# compared to a language with a garbage collector like Python.
...
Globals are a little bit different since we would need to talk about
pointers (which is a topic for Level 2) and you don't actually use
globals ever. Do you?
