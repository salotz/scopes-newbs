...
#+begin_src scopes :results drawer
...
Above we showed how to get pointers from ~local~ (stack) and ~global~
(data segment) allocated values. Because these are not heap allocated
they do have some extra rules surrounding their use.
...
#+begin_src org
2
$rironipegilugal:(mutable@ (storage = 'Function) i32)
32546
#+end_src

****** COMMENT Heap Pointers
