...
Scopes is not an object-oriented language and does not provide the
typical inheritance features found in them, such as multiple
inheritance.

However the simple form of inheritance (which we call simply
subtyping) is able to allow for the sharing of type methods in the
subtypes.
