...
Notice that this method really hasn't changed and you can still use it
as an object method. However, by convention it seems like a good idea
to distinguish class methods by using the Python convention of calling
the first argument ~cls~ (for class) as opposed to ~self~.
